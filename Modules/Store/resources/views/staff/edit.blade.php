...
@php
$title = __('store.edit_staff_permissions') . ' - ' . $user->name . ' - ' . config('app.name');
$pageTitle = __('store.edit_staff_permissions') . ': ' . $user->name;
@endphp

@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">{{ __('store.edit_staff_permissions') }}: {{ $user->name }}</h3>
        ...
        <div class="card-body">
            <form action="{{ route('store.staff.update', [$store, $user]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    <div class="d-flex align-items-center mb-4">
                        <div class="symbol symbol-50px me-3">
                            <div class="symbol-label fs-2 fw-bold bg-primary text-white">{{ substr($user->name, 0, 1) }}</div>
                        </div>
                        <div>
