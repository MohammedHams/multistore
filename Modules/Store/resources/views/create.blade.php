...
@php
$title = __('store.create_store') . ' - ' . config('app.name');
$pageTitle = __('store.create_store');
@endphp

@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">{{ __('store.create_store') }}</h3>
            ...
            <form action="{{ route('store.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="d-flex justify-content-end">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> {{ __('store.create_store') }}
                    </button>
                </div>
            </form>
        </div>
