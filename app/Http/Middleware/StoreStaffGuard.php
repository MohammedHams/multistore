...
class StoreStaffGuard
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // Check if user has the store-staff role or higher roles (admin, store-owner can access all routes)
        if (!Auth::user()->hasAnyRole(['admin', 'store-owner', 'store-staff'])) {
            return redirect()->route('access.denied')->with('error', 'يجب أن تكون موظف متجر للوصول إلى هذه الصفحة.');
        }
        
        // Admin can access all stores, so we only need to check permissions for store staff
        if (Auth::user()->hasRole('admin')) {
            return $next($request);
        }
        
        // If store_id is in the route parameters, check if the user has access to this store
        if ($request->route('store_id') || $request->input('store_id')) {
            $storeId = $request->route('store_id') ?? $request->input('store_id');
            $store = Store::find($storeId);
            
            // Store owners can access their own stores without additional permission checks
            if (Auth::user()->hasRole('store-owner') && Auth::user()->hasPermissionTo('manage-store-' . $storeId)) {
                return $next($request);
            }
            
            // Check if the store exists and if the user has permission to access this store
            if (!$store || !Auth::user()->hasPermissionTo('access-store-' . $storeId)) {
